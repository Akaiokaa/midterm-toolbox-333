...
public class Toolbox {
  ...
  public static int length(SingleNode head) {
    ...
    /*
     set our head to current,
     loop through our current going through each on
      increment a counter per node we find
      return increment
    */
    int counter = 0;
  ...
  public static DoubleNode findHead(DoubleNode tail) {
    ...
    loop through backwards( current.prev )
        current =  current.prev
    
    return current

    o(1) space 
    o(n) time where n = length of our entire doubleNode 
    */
